...
import { Client_CloudflareD1 } from "../d1-knex-dialect";

export class CloudflareD1Connection extends AbstractSqlConnection {
  ...
  async connect(): Promise<void> {
    ...
    this.client = this.createKnexClient("cloudflare-d1");
  ...
  private getPatchedDialect() {
    ...
    Sqlite3Dialect.prototype.processResponse = (obj: any, runner: any) => {
      if (
        obj.method === "raw" &&
        obj.sql.trim().match(CloudflareD1Connection.RUN_QUERY_RE)
      ) {
  ...
  private getCallMethod(obj: any): string {
    if (
      obj.method === "raw" &&
      obj.sql.trim().match(CloudflareD1Connection.RUN_QUERY_RETURNING)
    ) {
      return "all";
    }
