...
import { CloudflareD1SchemaHelper } from "./D1SchemaHelper";
import { CloudflareD1ExceptionConverter } from "./D1ExceptionConverter";

export class CloudflareD1Platform extends AbstractSqlPlatform {
  protected readonly schemaHelper: CloudflareD1SchemaHelper =
    new CloudflareD1SchemaHelper(this);
  protected readonly exceptionConverter = new CloudflareD1ExceptionConverter();
  ...
  getSearchJsonPropertyKey(
    ...
    if (aliased) {
      return expr(
        (alias) =>
          `json_extract(${this.quoteIdentifier(`${alias}.${a}`)}, '$.${b.join(
            "."
          )}')`
      );
    }
